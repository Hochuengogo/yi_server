...
add(STrigger = #s_trigger{triggers = TriggerList, next_id = NextId}, AddTrigger = #trigger{event = Event, callback = {_, _, _}}) ->
    ...
    NewTriggerList =
        case lists:keyfind(Event, 1, TriggerList) of
            {_, Triggers} ->
                NewTriggers = lists:reverse([NewAddTrigger | lists:reverse(Triggers)]), %% 按照放入的顺序
...
fire(STrigger, State, []) ->
    {STrigger, State};
fire(STrigger, State, [EventTuple | List]) ->
    {NewSTrigger, NewState} = fire(STrigger, State, EventTuple),
    fire(NewSTrigger, NewState, List);
